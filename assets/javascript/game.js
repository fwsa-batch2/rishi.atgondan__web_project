function play(x) {
    window.location.href = "/rishi.atgondan__web_project/pages/game.html"
}

let str = "0";

function teddy1(x) {
    document.getElementById('img1').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy2() {
    document.getElementById('img2').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy3() {
    document.getElementById('img3').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy4() {
    document.getElementById('img4').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy5() {
    document.getElementById('img5').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy6() {
    document.getElementById('img6').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy7() {
    document.getElementById('img7').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy8() {
    document.getElementById('img8').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy9() {
    document.getElementById('img9').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy10() {
    document.getElementById('img10').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";

    }
}
function teddy11() {
    document.getElementById('img11').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy12() {
    document.getElementById('img12').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy13() {
    document.getElementById('img13').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }

}
function teddy14() {
    document.getElementById('img14').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy15() {
    document.getElementById('img15').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}
function teddy16() {
    document.getElementById('img16').style.visibility = "hidden";
    let audio = new Audio('/rishi.atgondan__web_project/assets/images/gunsound.mp3');
    audio.play();
    let num = parseFloat(str) + 100;
    document.getElementById("points").innerHTML = num;
    if (num < 1600 && num > 0) {
        str = JSON.stringify(num);
    } else if (num == 1600) {
        document.getElementById("winner1").style.display = "block";
        document.getElementById("winner2").style.display = "block";
        let audio1 = new Audio('/rishi.atgondan__web_project/assets/images/claps.wav');
        audio1.play();
        let audio2 = new Audio('/rishi.atgondan__web_project/assets/images/rocket.wav');
        audio2.play();
        document.getElementById('prize').style.display = "block";
        document.getElementById('collect').style.display = "block";
    }
}

function redirect(x) {
    if (x == "collect")
        window.location.href = "/rishi.atgondan__web_project/pages/gift.html"
}
redirect(x);












